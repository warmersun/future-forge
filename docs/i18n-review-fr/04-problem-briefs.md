# French review — problem briefs (`locales/fr/problem-briefs.json`)

> Generated for human review of French localization.  
> **Edit only the FR text.** Keep keys (`` `like.this` ``) unchanged.  
> Date: 2026-08-09 · entries: 129

## How to correct

1. Edit **FR:** lines (or FR blocks) in place.
2. Optional: add a line `**NOTE:** ...` under an entry if you want a comment.
3. When finished, tell the agent to apply corrections from `docs/i18n-review-fr/`.



---

### `ag.currentState`

**EN:**

Industrial agriculture feeds billions but degrades soils, water, and biodiversity in many regions. Regenerative and precision practices are spreading, still minority share of global acreage.

**FR:**

L’agriculture industrielle nourrit des milliards de personnes mais dégrade les sols, l’eau et la biodiversité dans de nombreuses régions. Les pratiques régénératives et de précision se répandent, encore minoritaires dans les surfaces mondiales.

---
### `ag.rootCauses`

**EN:**

Short-term yield incentives, input lock-in, land inequality, climate stress, and markets that underprice ecological damage.

**FR:**

Incitations aux rendements à court terme, blocage des intrants, inégalités foncières, stress climatique et marchés qui sous-évaluent les dommages écologiques.

---
### `ag.warnings`

**EN:**

High-tech farming that only works for large capital can lock out smallholders. Fit inventions to local soils, water, and farmer knowledge—not one-size export packages.

**FR:**

Une agriculture de haute technologie qui ne fonctionne que pour le grand capital peut exclure les petits exploitants. Adaptez les inventions aux sols, à l’eau et aux connaissances des agriculteurs locaux – et non à des packages d’exportation de taille unique.

---
### `ageing.currentState`

**EN:**

Lifespans lengthened; healthspan did not always keep pace. Longevity science is active, but most gains still come from public health, chronic-disease care, and social determinants—not miracle pills.

**FR:**

Durée de vie allongée ; La durée de vie n'a pas toujours suivi le rythme. La science de la longévité est active, mais la plupart des gains proviennent toujours de la santé publique, des soins aux maladies chroniques et des déterminants sociaux – et non de pilules miracles.

---
### `ageing.rootCauses`

**EN:**

Biological aging processes, lifestyle and environment, and systems built for shorter lives and acute care rather than long multimorbidity.

**FR:**

Processus de vieillissement biologique, mode de vie et environnement, et systèmes conçus pour des vies plus courtes et des soins aigus plutôt que pour une multimorbidité longue.

---
### `ageing.warnings`

**EN:**

Anti-aging hype outruns evidence. Local inventing should target function, mobility, and care access—not claims of reversing aging this decade for a whole city.

**FR:**

Le battage médiatique anti-âge dépasse les preuves. L’invention locale devrait cibler la fonction, la mobilité et l’accès aux soins – et non prétendre inverser le vieillissement de l’ensemble d’une ville au cours de cette décennie.

---
### `air.currentState`

**EN:**

Ambient and household air pollution drive large disease burdens, especially near traffic, industry, and solid-fuel cooking. Monitoring improved; clean transitions are uneven.

**FR:**

La pollution de l’air ambiant et domestique entraîne d’importantes charges de morbidité, en particulier à proximité des zones de circulation, des industries et des cuisines à combustible solide. Suivi amélioré ; les transitions propres sont inégales.

---
### `air.rootCauses`

**EN:**

Fossil combustion, industrial emissions, dust, and indoor solid fuels—plus weak regulation and exposure inequality by neighborhood income.

**FR:**

Combustion fossile, émissions industrielles, poussières et combustibles solides intérieurs, auxquels s’ajoutent une réglementation faible et des inégalités d’exposition selon le revenu du quartier.

---
### `air.warnings`

**EN:**

Sensors alone do not clean air. Couple measurement with enforceable sources cuts, clean cooking, and mobility shifts people can actually use.

**FR:**

Les capteurs seuls ne purifient pas l’air. Associez la mesure à des réductions de sources exécutoires, à une cuisine propre et à des changements de mobilité que les gens peuvent réellement utiliser.

---
### `alzheimer.currentState`

**EN:**

Alzheimer’s and related dementias grow with aging populations. Disease-modifying options are limited and expensive; most care is family- and community-borne under high burden.

**FR:**

La maladie d’Alzheimer et les démences associées augmentent avec le vieillissement de la population. Les options de modification de la maladie sont limitées et coûteuses ; la plupart des soins sont pris en charge par la famille et la communauté, sous une lourde charge.

---
### `alzheimer.rootCauses`

**EN:**

Complex neurodegenerative biology, late diagnosis, limited therapies, and care systems unprepared for long progressive illness.

**FR:**

Biologie neurodégénérative complexe, diagnostic tardif, thérapies limitées et systèmes de soins non préparés à une maladie évolutive de longue durée.

---
### `alzheimer.warnings`

**EN:**

Surveillance framed as “safety” can strip dignity. Invent for caregiver support, navigation, and respectful assistance—not only tracking devices.

**FR:**

La surveillance présentée comme « sécurité » peut détruire la dignité. Inventez le soutien aux soignants, la navigation et l’assistance respectueuse, et pas seulement les dispositifs de suivi.

---
### `amr.currentState`

**EN:**

Bacteria and other microbes are evolving resistance to antibiotics and antimicrobials. Common infections and surgery become harder to treat; last-line drugs fail in hospitals while community misuse and farm use accelerate resistance. This is not a virus-outbreak theme—it is the erosion of antimicrobial effectiveness.

**FR:**

Les bactéries et autres microbes développent une résistance aux antibiotiques et aux antimicrobiens. Les infections courantes et la chirurgie deviennent plus difficiles à traiter ; les médicaments de dernière intention échouent dans les hôpitaux tandis que leur utilisation abusive dans la communauté et dans les exploitations agricoles accélère la résistance. Il ne s’agit pas d’un thème d’épidémie virale, mais plutôt de l’érosion de l’efficacité des antimicrobiens.

---
### `amr.rootCauses`

**EN:**

Overuse and misuse of antibiotics in humans and animals, weak diagnostics that force blind treatment, poor infection control, and underinvestment in new antimicrobials and stewardship.

**FR:**

Surutilisation et mauvaise utilisation des antibiotiques chez les humains et les animaux, diagnostics faibles qui obligent à un traitement à l’aveugle, mauvais contrôle des infections et sous-investissement dans les nouveaux antimicrobiens et dans la gestion.

---
### `amr.warnings`

**EN:**

Do not invent “another viral pandemic app” here. Prefer stewardship, rapid bacterial diagnostics, farm and pharmacy practice change, and access to the right drug—not unrestricted antibiotic vending. Preserve efficacy; do not invent systems that encourage more blind prescribing.

**FR:**

N’inventez pas ici « une autre application de pandémie virale ». Préférez la gestion responsable, les diagnostics bactériens rapides, le changement des pratiques agricoles et pharmaceutiques et l’accès au bon médicament, et non la vente sans restriction d’antibiotiques. Préserver l’efficacité ; n’inventez pas de systèmes qui encouragent davantage de prescriptions aveugles.

---
### `asteroid.currentState`

**EN:**

Near-Earth object surveys have improved dramatically; no known civilization-ending impact is imminent on short timescales, but incomplete catalogs and limited deflection readiness mean planetary defense is unfinished work.

**FR:**

Les enquêtes sur les objets géocroiseurs se sont considérablement améliorées ; aucun impact connu sur la fin de la civilisation n’est imminent à court terme, mais des catalogues incomplets et une préparation à la déviation limitée signifient que la défense planétaire est un travail inachevé.

---
### `asteroid.rootCauses`

**EN:**

Vast search space, limited telescope and follow-up capacity historically, and rare-event underinvestment. Deflection tech is maturing from tests, not yet a standing global service.

**FR:**

Vaste espace de recherche, capacité de télescope et de suivi historiquement limitée et sous-investissement pour les événements rares. La technologie de déflexion est en train de mûrir à partir de tests et ne constitue pas encore un service mondial permanent.

---
### `asteroid.warnings`

**EN:**

Local inventing is usually about sensing, public warning, and civil resilience—not city-scale deflection beams. Avoid sci-fi overclaim; focus on detection partnerships, drills, and recovery logistics if this theme is your frame.

**FR:**

L’invention locale porte généralement sur la détection, l’alerte publique et la résilience civile, et non sur les faisceaux de déviation à l’échelle de la ville. Évitez les excès de science-fiction ; Concentrez-vous sur les partenariats de détection, les exercices et la logistique de récupération si ce thème vous intéresse.

---
### `automation.currentState`

**EN:**

Automation and AI already reshape warehouses, offices, transport, and customer service. Some jobs vanish or deskill; new ones appear unevenly. Debates on UBI, reskilling, and shorter work weeks are live policy fights.

**FR:**

L'automatisation et l'IA remodèlent déjà les entrepôts, les bureaux, les transports et le service client. Certains emplois disparaissent ou déqualifient ; les nouveaux apparaissent de manière inégale. Les débats sur l’UBI, la reconversion professionnelle et la réduction des semaines de travail sont des combats politiques en direct.

---
### `automation.rootCauses`

**EN:**

Capital substitutes for routine labor where profitable; skills pipelines lag; social insurance was built for 20th-century employment; firms capture productivity gains without sharing.

**FR:**

Le capital remplace le travail de routine lorsque cela est rentable ; les réserves de compétences sont à la traîne ; l’assurance sociale a été construite pour l’emploi du XXe siècle ; les entreprises captent les gains de productivité sans les partager.

---
### `automation.warnings`

**EN:**

“Retrain everyone with an app” fails without real wages and placement. Invent for dignity, portable skills, and income bridges—not only efficiency for owners.

**FR:**

« Recycler tout le monde avec une application » échoue sans salaires réels ni placement. Inventez pour la dignité, les compétences transférables et les passerelles de revenus, et pas seulement pour l’efficacité des propriétaires.

---
### `cancer.currentState`

**EN:**

Survival has improved for many cancers where screening and therapy are available; late diagnosis and unequal access still drive preventable deaths, especially in lower-resource regions.

**FR:**

La survie s'est améliorée pour de nombreux cancers pour lesquels le dépistage et le traitement sont disponibles ; un diagnostic tardif et un accès inégal entraînent toujours des décès évitables, en particulier dans les régions à faibles ressources.

---
### `cancer.rootCauses`

**EN:**

Biology of uncontrolled cell growth, aging populations, risk factors (tobacco, infections, pollution), and health-system bottlenecks in imaging, pathology, and treatment capacity.

**FR:**

Biologie de la croissance cellulaire incontrôlée, du vieillissement des populations, des facteurs de risque (tabac, infections, pollution) et des goulots d'étranglement du système de santé en matière d'imagerie, de pathologie et de capacité de traitement.

---
### `cancer.warnings`

**EN:**

Do not invent “cure all cancer overnight.” Prefer earlier detection, triage, navigation, and access. Clinical claims need pilot honesty and partners.

**FR:**

N’inventez pas « guérir tous les cancers du jour au lendemain ». Préférez une détection, un tri, une navigation et un accès plus précoces. Les allégations cliniques nécessitent l’honnêteté du pilote et des partenaires.

---
### `chem-bio.currentState`

**EN:**

Chemical and biological weapons are banned under major treaties, yet dual-use research, lab accidents, and illicit intent remain risks. Detection, attribution, and public-health readiness are uneven worldwide.

**FR:**

Les armes chimiques et biologiques sont interdites par les principaux traités, mais la recherche à double usage, les accidents de laboratoire et les intentions illicites restent des risques. La détection, l’attribution et l’état de préparation de la santé publique sont inégaux à travers le monde.

---
### `chem-bio.rootCauses`

**EN:**

Knowledge and tools spread with legitimate science; verification is imperfect; norms can erode; preparedness funding rises after crises then fades; dual-use incentives in research and industry.

**FR:**

Connaissances et outils diffusés avec la science légitime ; la vérification est imparfaite ; les normes peuvent s’éroder ; le financement de la préparation augmente après les crises, puis diminue ; incitations au double usage dans la recherche et l’industrie.

---
### `chem-bio.warnings`

**EN:**

Do not invent systems that make high-risk pathogens or toxins easier to produce. Prefer detection, PPE, medical countermeasures, and transparent governance. Dual-use claims need strict red lines in your how-it-works.

**FR:**

N’inventez pas de systèmes qui facilitent la production d’agents pathogènes ou de toxines à haut risque. Préférez la détection, les EPI, les contre-mesures médicales et une gouvernance transparente. Les allégations de double usage nécessitent des lignes rouges strictes dans votre fonctionnement.

---
### `child.currentState`

**EN:**

Child mortality fell dramatically, yet preventable deaths from pneumonia, diarrhea, malaria, and malnutrition continue where primary care and vaccines lag. Nutrition and early development gaps shape lifelong outcomes.

**FR:**

La mortalité infantile a chuté de façon spectaculaire, mais les décès évitables dus à la pneumonie, à la diarrhée, au paludisme et à la malnutrition persistent là où les soins primaires et les vaccins sont à la traîne. Les écarts en matière de nutrition et de développement précoce déterminent les résultats tout au long de la vie.

---
### `child.rootCauses`

**EN:**

Poverty, weak primary care, vaccine and nutrition access gaps, water/sanitation failures, and maternal health links that start before birth.

**FR:**

Pauvreté, soins primaires faibles, déficits d’accès aux vaccins et à la nutrition, échecs en matière d’eau et d’assainissement et liens de santé maternelle qui commencent avant la naissance.

---
### `child.warnings`

**EN:**

Family-facing inventions need caregiver trust and cold-chain reality. Avoid data collection without care capacity to act on results.

**FR:**

Les inventions destinées aux familles nécessitent la confiance des soignants et la réalité de la chaîne du froid. Évitez la collecte de données sans la capacité des soins à agir sur les résultats.

---
### `cities.currentState`

**EN:**

Most humans will live in urban areas. Cities concentrate opportunity and inequality, heat islands, congestion, and service stress—while also hosting innovation capacity.

**FR:**

La plupart des humains vivront dans des zones urbaines. Les villes concentrent les opportunités et les inégalités, les îlots de chaleur, les embouteillages et le stress lié aux services, tout en abritant également une capacité d’innovation.

---
### `cities.rootCauses`

**EN:**

Rapid urbanization, land and transport policy failures, fragmented governance across metro edges, and infrastructure lagging population growth.

**FR:**

Urbanisation rapide, échecs des politiques foncières et de transport, gouvernance fragmentée à la périphérie des zones métropolitaines et infrastructures à la traîne de la croissance démographique.

---
### `cities.warnings`

**EN:**

Smart-city stacks without democratic oversight become surveillance. Prefer public goods, mobility, and housing equity over gadget dashboards.

**FR:**

Les piles de villes intelligentes sans contrôle démocratique deviennent de la surveillance. Préférez les biens publics, la mobilité et la valeur du logement aux tableaux de bord de gadgets.

---
### `climate.currentState`

**EN:**

Global temperatures and extreme weather trends are rising with cumulative emissions. Clean energy is scaling fast in many markets, yet fossil systems, lock-in, and adaptation gaps remain large.

**FR:**

Les températures mondiales et les tendances météorologiques extrêmes augmentent avec les émissions cumulées. L’énergie propre se développe rapidement sur de nombreux marchés, mais les systèmes fossiles, le verrouillage et les écarts d’adaptation restent importants.

---
### `climate.rootCauses`

**EN:**

Fossil-based energy and industry, land-use change, and delayed collective action under free-rider incentives. Adaptation is underfunded relative to mitigation talk.

**FR:**

Énergie et industrie basées sur les fossiles, changement d’affectation des terres et action collective retardée dans le cadre d’incitations aux resquilleurs. L’adaptation est sous-financée par rapport aux discours sur l’atténuation.

---
### `climate.warnings`

**EN:**

Local climate inventions should cut real exposure or emissions locally—not offset theater. Timing: deploy near-term resilience while longer transitions run.

**FR:**

Les inventions climatiques locales devraient réduire l’exposition ou les émissions réelles au niveau local, et non compenser le théâtre. Timing : déployer une résilience à court terme pendant que des transitions plus longues se déroulent.

---
### `coord.currentState`

**EN:**

Cross-border and cross-city problems (storms, outbreaks, supply shocks) still meet fragmented budgets, data, and response funds. Pilots cooperate; standing shared capacity is rare.

**FR:**

Les problèmes transfrontaliers et interurbains (tempêtes, épidémies, chocs d’approvisionnement) se heurtent encore à des budgets, des données et des fonds de réponse fragmentés. Les pilotes coopèrent ; la capacité partagée permanente est rare.

---
### `coord.rootCauses`

**EN:**

Sovereignty instincts, free-rider incentives, incompatible systems, and short political horizons that underfund shared early warning and pooled response.

**FR:**

Des instincts de souveraineté, des incitations à un passager clandestin, des systèmes incompatibles et des horizons politiques courts qui sous-financent l’alerte précoce partagée et la réponse commune.

---
### `coord.warnings`

**EN:**

A shared dashboard without a shared fund or decision rights is theater. Design governance and incentives as carefully as the sensors.

**FR:**

Un tableau de bord partagé sans fonds partagé ni droits de décision est un théâtre. Concevez la gouvernance et les incitations avec autant de soin que les capteurs.

---
### `eco.currentState`

**EN:**

Biodiversity loss, deforestation, ocean degradation, and pollution continue at rates that threaten ecosystem services humans depend on. Protected areas and restoration efforts expand but lag drivers of loss.

**FR:**

La perte de biodiversité, la déforestation, la dégradation des océans et la pollution se poursuivent à des rythmes qui menacent les services écosystémiques dont dépendent les humains. Les zones protégées et les efforts de restauration se développent mais sont à la traîne des facteurs de perte.

---
### `eco.rootCauses`

**EN:**

Habitat conversion, overharvest, invasive species, pollution, and climate change—driven by economic systems that externalize nature’s value.

**FR:**

Conversion des habitats, surexploitation, espèces envahissantes, pollution et changement climatique – entraînés par des systèmes économiques qui externalisent la valeur de la nature.

---
### `eco.warnings`

**EN:**

“Green” inventions can greenwash extractive projects. Measure real habitat and community outcomes. Avoid claiming full ecosystem recovery on short game timelines.

**FR:**

Les inventions « vertes » peuvent écologiser les projets extractifs. Mesurez les résultats réels en matière d’habitat et de communauté. Évitez de revendiquer une récupération complète de l’écosystème sur des délais de jeu courts.

---
### `education.currentState`

**EN:**

School enrollment rose worldwide, but learning outcomes, teacher shortages, and digital divides leave huge talent unrealized. Crises and poverty still pull children out of classrooms.

**FR:**

La scolarisation a augmenté dans le monde entier, mais les résultats d’apprentissage, la pénurie d’enseignants et la fracture numérique laissent inexploités d’énormes talents. Les crises et la pauvreté continuent de retirer les enfants des salles de classe.

---
### `education.rootCauses`

**EN:**

Underfunded schools, uneven teacher quality and pay, language and disability barriers, connectivity gaps, and opportunity costs of child labor or care work.

**FR:**

Écoles sous-financées, qualité et rémunération inégales des enseignants, barrières linguistiques et liées au handicap, déficits de connectivité et coûts d'opportunité du travail des enfants ou des soins aux enfants.

---
### `education.warnings`

**EN:**

AI tutors without teachers can widen gaps if only the well-connected benefit. Prefer tools that support educators, offline use, and local curricula—not pure replacement fantasies.

**FR:**

Les tuteurs d’IA sans enseignants peuvent creuser les écarts ne serait-ce que grâce à l’avantage d’être bien connecté. Préférez les outils qui prennent en charge les enseignants, l'utilisation hors ligne et les programmes locaux, et non de purs fantasmes de remplacement.

---
### `energy-access.currentState`

**EN:**

Hundreds of millions lack electricity or reliable supply; many more face costly, intermittent power. Solar and mini-grids expand access, still incomplete for productive uses and clinics.

**FR:**

Des centaines de millions de personnes manquent d’électricité ou d’approvisionnement fiable ; beaucoup d’autres sont confrontés à une alimentation électrique coûteuse et intermittente. L’énergie solaire et les mini-réseaux élargissent l’accès, encore incomplet pour les usages productifs et les cliniques.

---
### `energy-access.rootCauses`

**EN:**

High grid extension costs, weak utilities, poverty, and fuels that are dirty or expensive at the margin. Policy and finance often lag technical possibility.

**FR:**

Coûts élevés d’extension du réseau, faiblesse des services publics, pauvreté et carburants qui sont sales ou chers à la marge. La politique et la finance sont souvent à la traîne des possibilités techniques.

---
### `energy-access.warnings`

**EN:**

First watts that die at dusk fail clinics. Pair generation with storage, maintenance training, and realistic load priorities.

**FR:**

Les premiers watts qui meurent au crépuscule échouent dans les cliniques. Associez la génération au stockage, à la formation à la maintenance et à des priorités de charge réalistes.

---
### `fgm.currentState`

**EN:**

More than 200 million girls and women alive today have undergone FGM; millions more remain at risk each year. Prevalence has fallen in many countries—a girl is about one-third less likely to undergo FGM than thirty years ago—yet progress is uneven and COVID-era disruptions set programs back.

**FR:**

Plus de 200 millions de filles et de femmes en vie aujourd’hui ont subi des MGF ; des millions d’autres restent menacés chaque année. La prévalence a diminué dans de nombreux pays – une fille a environ un tiers moins de risques de subir une MGF qu’il y a trente ans – mais les progrès sont inégaux et les perturbations liées à la COVID-19 ont fait reculer les programmes.

---
### `fgm.rootCauses`

**EN:**

Social norms, marriageability pressure, practitioner livelihoods, weak enforcement, and cross-border “cutting season” mobility. Attitudes are shifting in many places where girls and women who know of FGM say it should end, but practice can lag belief.

**FR:**

Normes sociales, pression sur le mariage, moyens de subsistance des praticiens, faible application et mobilité transfrontalière « pendant la saison des coupes ». Les attitudes changent dans de nombreux endroits où les filles et les femmes qui connaissent les MGF affirment qu'elles devraient mettre fin, mais la pratique peut être à la traîne.

---
### `fgm.warnings`

**EN:**

Invent for community-led abandonment, girls’ education and protection, and survivor care—never graphic “rescue” theater or shaming gadgets. Do not medicalize FGM as a clinical upgrade. Center local women’s leadership, confidentiality, and dignity.

**FR:**

Inventez pour l’abandon dirigé par la communauté, l’éducation et la protection des filles et les soins aux survivantes – jamais de théâtre de « sauvetage » graphique ou de gadgets humiliants. Ne médicalisez pas les MGF comme une amélioration clinique. Centrer le leadership, la confidentialité et la dignité des femmes locales.

---
### `food.currentState`

**EN:**

The world produces enough calories overall, yet hunger and malnutrition persist through poverty, conflict, waste, and poor distribution. Price shocks hit low-income households first.

**FR:**

Le monde produit globalement suffisamment de calories, mais la faim et la malnutrition persistent à travers la pauvreté, les conflits, le gaspillage et une mauvaise répartition. Les chocs de prix frappent en premier lieu les ménages à faible revenu.

---
### `food.rootCauses`

**EN:**

Access and affordability, not only total production; supply-chain fragility; conflict; climate hits to harvests; and diets skewed by cheap ultra-processed calories.

**FR:**

Accès et abordabilité, pas seulement la production totale ; fragilité de la chaîne d'approvisionnement; conflit; le climat affecte les récoltes ; et des régimes alimentaires biaisés par des calories ultra-transformées bon marché.

---
### `food.warnings`

**EN:**

Food-tech that prices out the hungry is a photo-op. Prioritize cold chain, local production, and affordability—not only novel proteins for wealthy markets.

**FR:**

La technologie alimentaire qui met le prix sur les affamés est une séance de photos. Donner la priorité à la chaîne du froid, à la production locale et à un prix abordable – et pas seulement aux nouvelles protéines destinées aux marchés riches.

---
### `genocide.currentState`

**EN:**

Mass atrocity risk remains real in several regions. Early-warning networks, satellite monitoring, and civil-society alerts exist, but response is often late, politicized, or underfunded. Prevention is possible in theory; practice still fails when will and coordination fail.

**FR:**

Le risque d’atrocités de masse reste réel dans plusieurs régions. Des réseaux d’alerte précoce, une surveillance par satellite et des alertes de la société civile existent, mais la réponse est souvent tardive, politisée ou sous-financée. La prévention est possible en théorie ; la pratique échoue toujours lorsque la volonté et la coordination échouent.

---
### `genocide.rootCauses`

**EN:**

Identity conflict, dehumanizing propaganda, weak institutions, arms flows, and international free-riding. Information arrives, but incentives to act early are weak when costs are immediate and benefits diffuse.

**FR:**

Conflit d’identité, propagande déshumanisante, institutions faibles, flux d’armes et parasitisme international. Les informations arrivent, mais les incitations à agir rapidement sont faibles lorsque les coûts sont immédiats et les bénéfices diffus.

---
### `genocide.warnings`

**EN:**

Tech for surveillance or targeting can dual-use into oppression. Prioritize civilian protection, verified information, and community legitimacy—not purely automated “threat scores.” Local inventions should not replace political accountability with black-box alerts.

**FR:**

Les technologies de surveillance ou de ciblage peuvent avoir une double utilité et conduire à l’oppression. Donnez la priorité à la protection des civils, aux informations vérifiées et à la légitimité de la communauté, et non à des « scores de menace » purement automatisés. Les inventions locales ne devraient pas remplacer la responsabilité politique par des alertes de type boîte noire.

---
### `homeless.currentState`

**EN:**

Visible homelessness and housing insecurity are rising in many cities even in wealthy countries. Shelter systems are overstretched; exits to stable housing lag inflow.

**FR:**

Le sans-abrisme visible et l’insécurité du logement augmentent dans de nombreuses villes, même dans les pays riches. Les systèmes de refuges sont débordés ; les sorties vers des logements stables accusent un retard dans l’afflux.

---
### `homeless.rootCauses`

**EN:**

Housing costs outpacing wages, mental health and substance gaps, weak tenancy protections, and underbuilt social housing—plus discharges from institutions without housing plans.

**FR:**

Des coûts de logement supérieurs aux salaires, des écarts en matière de santé mentale et de toxicomanie, de faibles protections de location et des logements sociaux sous-construits, sans compter les sorties d'institutions sans plan de logement.

---
### `homeless.warnings`

**EN:**

Clearance and pure tech tracking without housing supply worsen harm. Invent for shelter quality, services linkage, and paths to stay housed.

**FR:**

Le dédouanement et le suivi technologique pur sans offre de logements aggravent les dommages. Inventez la qualité des abris, les liens entre les services et les moyens de rester logé.

---
### `infectious.currentState`

**EN:**

Pandemic readiness improved in tools (sequencing, vaccines platforms) after COVID-19, but funding, trust, and equity in access remain fragile. Endemic diseases still kill millions yearly where systems are weak.

**FR:**

La préparation à la pandémie s’est améliorée grâce aux outils (séquençage, plateformes de vaccins) après la COVID-19, mais le financement, la confiance et l’équité d’accès restent fragiles. Les maladies endémiques tuent encore des millions de personnes chaque année là où les systèmes sont faibles.

---
### `infectious.rootCauses`

**EN:**

Zoonotic spillover, dense travel, weak primary care, antimicrobial resistance, misinformation, and underfunded surveillance in high-risk regions.

**FR:**

Répercussions zoonotiques, déplacements denses, soins primaires faibles, résistance aux antimicrobiens, désinformation et surveillance sous-financée dans les régions à haut risque.

---
### `infectious.warnings`

**EN:**

Outbreak tech without trust fails. Pair detection with communication and care capacity. Do not invent forced quarantine black boxes that destroy legitimacy.

**FR:**

La technologie anti-épidémie sans confiance échoue. Associez la détection à la capacité de communication et de soins. N’inventez pas de boîtes noires de quarantaine forcée qui détruisent la légitimité.

---
### `maternal.currentState`

**EN:**

Most maternal deaths are preventable with skilled care, emergency transport, and basic supplies—yet disparities remain huge between and within countries.

**FR:**

La plupart des décès maternels peuvent être évités grâce à des soins qualifiés, des transports d’urgence et des fournitures de base, mais les disparités restent énormes entre les pays et au sein de ceux-ci.

---
### `maternal.rootCauses`

**EN:**

Distance to facilities, shortages of midwives and blood/products, poverty, adolescent pregnancy, and care quality gaps including disrespectful treatment that deters use.

**FR:**

Distance des établissements, pénurie de sages-femmes et de produits sanguins/produits sanguins, pauvreté, grossesses chez les adolescentes et écarts de qualité des soins, y compris un traitement irrespectueux qui dissuade leur utilisation.

---
### `maternal.warnings`

**EN:**

Apps without ambulances and blood banks fail. Invent for referral, transport, and respectful skilled birth attendance—not only pregnancy trackers.

**FR:**

Les applications sans ambulances ni banques de sang échouent. Inventez des systèmes d’orientation, de transport et d’accouchements respectueux et qualifiés, et pas seulement des outils de suivi de grossesse.

---
### `memory.currentState`

**EN:**

Human memory is incomplete and unreliable even in healthy people. For the first time, ordinary people can create rich multimedia records of lived experience—yet most life is still uncaptured, unsearchable, or locked in formats that will rot.

**FR:**

La mémoire humaine est incomplète et peu fiable, même chez les personnes en bonne santé. Pour la première fois, les gens ordinaires peuvent créer de riches enregistrements multimédias de leurs expériences vécues, mais la majeure partie de la vie reste encore non capturée, impossible à rechercher ou enfermée dans des formats qui pourriront.

---
### `memory.rootCauses`

**EN:**

Biological forgetting; lack of affordable, private capture tools; organizational churn that erases institutional knowledge; cultural loss when elders’ stories are never recorded; digital obsolescence of media and formats.

**FR:**

Oubli biologique ; manque d'outils de capture privés et abordables ; un désabonnement organisationnel qui efface les connaissances institutionnelles ; perte culturelle lorsque les histoires des aînés ne sont jamais enregistrées ; obsolescence numérique des supports et des formats.

---
### `memory.warnings`

**EN:**

This is not Alzheimer or dementia care. Invent for documenting and preserving experience—personal, community, and institutional—with consent and privacy. Life-logging without control becomes surveillance. Prefer user-owned archives and dignified legacy, not forced total recall.

**FR:**

Il ne s’agit pas de soins pour la maladie d’Alzheimer ou la démence. Inventez pour documenter et préserver l’expérience (personnelle, communautaire et institutionnelle) avec le consentement et la confidentialité. L'enregistrement de la vie sans contrôle devient une surveillance. Préférez les archives appartenant aux utilisateurs et un héritage digne, et non un rappel total forcé.

---
### `mental.currentState`

**EN:**

Depression, anxiety, and related conditions are leading causes of disability worldwide. Demand for care exceeds supply of clinicians; stigma still blocks help-seeking in many communities.

**FR:**

La dépression, l’anxiété et les affections associées sont les principales causes d’invalidité dans le monde. La demande de soins dépasse l’offre de cliniciens ; la stigmatisation bloque toujours la recherche d’aide dans de nombreuses communautés.

---
### `mental.rootCauses`

**EN:**

Social isolation, economic stress, trauma, substance use, underfunded services, and models of care that are clinic-centric rather than community-embedded.

**FR:**

Isolement social, stress économique, traumatismes, consommation de substances, services sous-financés et modèles de soins centrés sur la clinique plutôt que intégrés à la communauté.

---
### `mental.warnings`

**EN:**

Chatbots are not a full clinical system. Design for crisis escalation paths, privacy, and human care—not isolation in an app that cannot intervene safely.

**FR:**

Les chatbots ne constituent pas un système clinique complet. Concevoir des voies d'escalade de crise, de confidentialité et de soins humains, et non l'isolement dans une application qui ne peut pas intervenir en toute sécurité.

---
### `mideast.currentState`

**EN:**

The Middle East holds layered conflicts, occupation dynamics, trauma, and fragile livelihoods alongside vibrant civil society, trade, and diplomacy efforts. Violence and displacement recur; peace processes are intermittent and contested.

**FR:**

Le Moyen-Orient abrite des conflits à plusieurs niveaux, des dynamiques d’occupation, des traumatismes et des moyens de subsistance fragiles, ainsi que des efforts dynamiques de la société civile, du commerce et de la diplomatie. La violence et les déplacements sont récurrents ; les processus de paix sont intermittents et contestés.

---
### `mideast.rootCauses`

**EN:**

Historical grievances, borders and resource competition, external interventions, authoritarian and factional politics, and economic exclusion that fuels recruitment and despair.

**FR:**

Griefs historiques, concurrence aux frontières et aux ressources, interventions extérieures, politiques autoritaires et factionnelles et exclusion économique qui alimente le recrutement et le désespoir.

---
### `mideast.warnings`

**EN:**

Local inventions should center livelihoods, trauma-aware care, and shared infrastructure—not “solve geopolitics” with a gadget. Avoid one-sided tech that increases surveillance or military asymmetry under a peace label.

**FR:**

Les inventions locales devraient se concentrer sur les moyens de subsistance, les soins tenant compte des traumatismes et les infrastructures partagées, et non « résoudre la géopolitique » avec un gadget. Évitez les technologies unilatérales qui augmentent la surveillance ou l’asymétrie militaire sous une étiquette de paix.

---
### `misinfo.currentState`

**EN:**

Shared facts and institutional trust are under pressure from algorithmic amplification, cheap synthetic media, and hollowed local journalism. Falsehoods travel faster than corrections; crises (health, elections, disasters) become epistemic emergencies as well as physical ones.

**FR:**

Les faits partagés et la confiance institutionnelle sont sous la pression de l’amplification algorithmique, des médias synthétiques bon marché et d’un journalisme local vide de sens. Les mensonges voyagent plus vite que les corrections ; les crises (santé, élections, catastrophes) deviennent des urgences épistémiques aussi bien que physiques.

---
### `misinfo.rootCauses`

**EN:**

Attention economies reward outrage; verification is costly; AI lowers the cost of convincing fakes; polarisation and weak media literacy erode common ground; underfunded public-interest media leave rumor unchallenged.

**FR:**

Attention, les économies récompensent l’indignation ; la vérification est coûteuse ; L’IA réduit le coût des contrefaçons convaincantes ; la polarisation et la faible éducation aux médias érodent les terrains d’entente ; Les médias d’intérêt public sous-financés laissent la rumeur sans contestation.

---
### `misinfo.warnings`

**EN:**

Do not invent “AI deletes wrongthink” as a clean win. Prefer provenance, local trusted intermediaries, and user agency. Surveillance framed as anti-disinfo can silence legitimate speech. Design for correction speed and dignity, not purity police.

**FR:**

N’inventez pas « l’IA supprime les idées fausses » comme une victoire nette. Préférez la provenance, les intermédiaires locaux de confiance et l’agence utilisatrice. La surveillance présentée comme anti-désinformation peut faire taire les discours légitimes. Conçu pour la rapidité et la dignité de la correction, pas pour la police de la pureté.

---
### `nuclear.currentState`

**EN:**

Nuclear weapons states retain arsenals; modernization continues; arms-control architecture is frayed. Risk is less “movie countdown” than miscalculation, accident, or crisis escalation under compressed decision times.

**FR:**

Les États dotés d’armes nucléaires conservent des arsenaux ; la modernisation se poursuit ; l’architecture du contrôle des armements est effilochée. Le risque est moins un « compte à rebours de film » qu’une erreur de calcul, un accident ou une escalade de crise dans des délais de décision compressés.

---
### `nuclear.rootCauses`

**EN:**

Security dilemmas, prestige, domestic politics of arsenals, imperfect early-warning and command systems, and erosion of treaties and norms.

**FR:**

Dilemmes de sécurité, prestige, politique intérieure des arsenaux, systèmes d’alerte précoce et de commandement imparfaits et érosion des traités et des normes.

---
### `nuclear.warnings`

**EN:**

Local inventing almost never means building weapons. Focus on crisis communication resilience, civil protection literacy, and de-escalation tools. Do not glamorize launch systems or invent as if municipal actors control the nuclear button.

**FR:**

Inventer localement ne signifie presque jamais fabriquer des armes. Concentrez-vous sur la résilience de la communication en cas de crise, les connaissances en matière de protection civile et les outils de désescalade. Ne glorifiez pas les systèmes de lancement et n’inventez pas comme si les acteurs municipaux contrôlaient le bouton nucléaire.

---
### `poverty.currentState`

**EN:**

Extreme poverty has fallen globally over decades but remains concentrated; hundreds of millions still lack reliable income, services, and agency. Progress is uneven; shocks (conflict, climate, disease) reverse gains quickly for the most vulnerable.

**FR:**

L’extrême pauvreté a diminué à l’échelle mondiale au fil des décennies, mais reste concentrée ; des centaines de millions de personnes manquent toujours de revenus, de services et d’agences fiables. Les progrès sont inégaux ; les chocs (conflits, climat, maladie) annulent rapidement les acquis pour les plus vulnérables.

---
### `poverty.rootCauses`

**EN:**

Exclusion from markets and services, weak property and labor rights, health and education gaps, geographic isolation, discrimination, and political voice gaps. Growth alone does not reach everyone without deliberate inclusion.

**FR:**

Exclusion des marchés et des services, faiblesse des droits de propriété et du travail, écarts en matière de santé et d’éducation, isolement géographique, discrimination et manque de voix politique. La croissance seule ne peut atteindre tout le monde sans une inclusion délibérée.

---
### `poverty.warnings`

**EN:**

Digital or fintech “solutions” can exclude people without IDs, connectivity, or literacy. Design for the last mile, cash + services where needed, and avoid debt traps or data extraction framed as help.

**FR:**

Les « solutions » numériques ou fintech peuvent exclure les personnes sans pièce d’identité, sans connectivité ou sans alphabétisation. Concevez pour le dernier kilomètre, offrez de l'argent et des services si nécessaire, et évitez les pièges de l'endettement ou l'extraction de données présentée comme une aide.

---
### `radicalization.currentState`

**EN:**

Violent extremism and radicalization still recruit through prejudice, online pipelines, and offline grievances. Prevention programs exist in schools and communities, but coverage is uneven and often reacts after harm rather than disarming pathways early.

**FR:**

L’extrémisme violent et la radicalisation continuent de recruter par le biais de préjugés, de réseaux en ligne et de griefs hors ligne. Des programmes de prévention existent dans les écoles et les communautés, mais leur couverture est inégale et ils réagissent souvent après un incident plutôt que de désarmer rapidement les voies de recours.

---
### `radicalization.rootCauses`

**EN:**

Identity conflict, dehumanizing propaganda, social exclusion, weak rule of law, and platforms that amplify grievance faster than dialogue. Young people are targeted early—on school benches and online—before offline violence.

**FR:**

Conflit d’identité, propagande déshumanisante, exclusion sociale, État de droit faible et plateformes qui amplifient les griefs plus rapidement que le dialogue. Les jeunes sont ciblés très tôt – sur les bancs d’école et en ligne – avant la violence hors ligne.

---
### `radicalization.warnings`

**EN:**

Disarming radicalization starts with human rights, dialogue across boundaries, and youth empowerment—not mass surveillance or predictive “terror scores.” Dual-use watchlists can become tools of discrimination. Prefer off-ramps, media literacy, and community legitimacy.

**FR:**

Désarmer la radicalisation commence par les droits de l’homme, le dialogue au-delà des frontières et l’autonomisation des jeunes – et non par la surveillance de masse ou les « scores de terrorisme » prédictifs. Les listes de surveillance à double usage peuvent devenir des outils de discrimination. Préférez les sorties, l’éducation aux médias et la légitimité communautaire.

---
### `refugees.currentState`

**EN:**

Tens of millions are forcibly displaced. Host cities absorb arrivals into strained housing, schools, and clinics. Legal status, work rights, and integration pathways vary widely and change with politics.

**FR:**

Des dizaines de millions de personnes sont déplacées de force. Les villes hôtes absorbent les arrivées dans des logements, des écoles et des cliniques en difficulté. Le statut juridique, les droits du travail et les parcours d’intégration varient considérablement et changent avec la politique.

---
### `refugees.rootCauses`

**EN:**

Conflict, persecution, climate stress, and failed asylum systems. Receiving countries underfund reception while origin crises persist.

**FR:**

Conflits, persécutions, stress climatique et systèmes d’asile défaillants. Les pays d’accueil sous-financent l’accueil alors que les crises à l’origine persistent.

---
### `refugees.warnings`

**EN:**

Biometric ID and tracking can become tools of exclusion. Design for dignity, language access, and housing/work—not warehouses of data without consent.

**FR:**

L’identification biométrique et le suivi peuvent devenir des outils d’exclusion. Concevoir pour la dignité, l’accès aux langues et le logement/travail – et non pour des entrepôts de données sans consentement.

---
### `reproductive.currentState`

**EN:**

Sexual and reproductive health means people can have a safe sex life and decide if, when, and how often to reproduce. Contraceptive access has improved globally but remains low in many regions; STIs, infertility care, and confidential services for all genders lag.

**FR:**

La santé sexuelle et reproductive signifie que les gens peuvent avoir une vie sexuelle sûre et décider si, quand et à quelle fréquence ils souhaitent se reproduire. L'accès à la contraception s'est amélioré à l'échelle mondiale mais reste faible dans de nombreuses régions ; Les IST, les soins contre l’infertilité et les services confidentiels pour tous les sexes sont à la traîne.

---
### `reproductive.rootCauses`

**EN:**

Stockouts, stigma, provider bias, cost, restrictive policy, and services designed only around pregnancy—not men, partners, adolescents, or fertility care. FGM and forced marriage are related harms but not the whole of RH.

**FR:**

Ruptures de stocks, stigmatisation, préjugés des prestataires, coûts, politiques restrictives et services conçus uniquement autour de la grossesse, et non des hommes, des partenaires, des adolescents ou des soins de fertilité. Les MGF et le mariage forcé sont des préjudices liés, mais pas l’ensemble de la SR.

---
### `reproductive.warnings`

**EN:**

This theme is for all people—not only women and children. Maternal emergency obstetric care is its own theme; FGM is its own theme. Invent for agency, privacy, STI care (including men), contraception choice, and fertility access—never coercive fertility control framed as good.

**FR:**

Ce thème s’adresse à tous, pas seulement aux femmes et aux enfants. Les soins obstétricaux maternels d’urgence constituent un thème à part entière ; Les MGF constituent un thème à part entière. Inventez pour le libre arbitre, la vie privée, les soins des IST (y compris les hommes), le choix de la contraception et l’accès à la fécondité – sans jamais considérer le contrôle coercitif de la fécondité comme étant une bonne chose.

---
### `rogue-si.currentState`

**EN:**

Frontier AI systems already outperform humans on many narrow tasks and are being wired into search, code, finance, and government pilots. Full “superintelligence” is not here, but rapid capability gains and competitive deployment mean control and alignment lag behind capability in many institutions.

**FR:**

Les systèmes Frontier AI surpassent déjà les humains dans de nombreuses tâches spécifiques et sont intégrés à des projets pilotes de recherche, de code, de finance et gouvernementaux. La « superintelligence » à part entière n’est pas là, mais les gains rapides de capacités et le déploiement compétitif signifient que le contrôle et l’alignement sont en retard par rapport aux capacités de nombreuses institutions.

---
### `rogue-si.rootCauses`

**EN:**

Race dynamics reward speed over safety; evaluation of open-ended systems is hard; incentives favor productization; governance is fragmented across labs, states, and open-source ecosystems; once systems act at scale, reverse is costly.

**FR:**

La dynamique de course récompense la vitesse plutôt que la sécurité ; l’évaluation des systèmes ouverts est difficile ; les incitations favorisent la production ; la gouvernance est fragmentée entre les laboratoires, les États et les écosystèmes open source ; une fois que les systèmes fonctionnent à grande échelle, l’inverse est coûteux.

---
### `rogue-si.warnings`

**EN:**

Local inventions that hand high-stakes decisions to opaque models can lock in harm before safeguards exist. Prefer human-in-the-loop, audit trails, and narrow scopes. Do not invent as if full autonomous AGI is already reliable infrastructure.

**FR:**

Les inventions locales qui confient des décisions à enjeux élevés à des modèles opaques peuvent bloquer les dommages avant que des garanties n’existent. Préférez l’humain dans la boucle, les pistes d’audit et les portées étroites. N’inventez pas comme si l’AGI entièrement autonome était déjà une infrastructure fiable.

---
### `rural-roads.currentState`

**EN:**

On the order of a billion rural people live more than about 2 km from an all-season road. Isolation from markets, clinics, and schools is a core poverty trap feature—“to be poor is to be isolated.” Rainy seasons and terrain cut access further.

**FR:**

Environ un milliard de ruraux vivent à plus de 2 km environ d’une route praticable en toutes saisons. L’isolement des marchés, des cliniques et des écoles est un élément essentiel du piège de la pauvreté : « être pauvre, c’est être isolé ». Les saisons des pluies et le relief réduisent encore davantage l'accès.

---
### `rural-roads.rootCauses`

**EN:**

Underinvestment in rural infrastructure, weak maintenance after ribbon-cutting, difficult terrain and climate, and political bias toward visible urban projects over last-mile connectivity.

**FR:**

Sous-investissement dans les infrastructures rurales, faible entretien après l’inauguration, terrain et climat difficiles, et parti pris politique en faveur de projets urbains visibles sur la connectivité du dernier kilomètre.

---
### `rural-roads.warnings`

**EN:**

A road without maintenance is a temporary promise. Invent for all-weather access, clinic and market logistics, and upkeep incentives—not only one-time construction theater. Watch displacement and land grabs framed as “corridor development.”

**FR:**

Une route sans entretien est une promesse temporaire. Inventez un accès par tous les temps, une logistique pour les cliniques et les marchés, ainsi que des incitations à l'entretien, et pas seulement un théâtre de construction ponctuel. Regardez les déplacements et les accaparements de terres présentés comme du « développement de corridors ».

---
### `sanitation.currentState`

**EN:**

Billions still lack safely managed sanitation; over a billion lack even basic private toilets or latrines, and hundreds of millions practice open defecation. Urban sewers lag informal growth; school toilets and fecal sludge systems remain weak links.

**FR:**

Des milliards de personnes ne disposent toujours pas d’assainissement géré en toute sécurité ; plus d’un milliard de personnes ne disposent même pas de toilettes ou de latrines privées de base, et des centaines de millions pratiquent la défécation à l’air libre. Les égouts urbains sont à la traîne de la croissance informelle ; les toilettes des écoles et les systèmes de traitement des boues fécales restent des maillons faibles.

---
### `sanitation.rootCauses`

**EN:**

Cost of toilets and sewers, underfunded utilities, stigma, land tenure barriers in informal settlements, and neglect of the full chain from latrine to safe treatment.

**FR:**

Coût des toilettes et des égouts, services publics sous-financés, stigmatisation, obstacles fonciers dans les quartiers informels et négligence de toute la chaîne allant des latrines au traitement sûr.

---
### `sanitation.warnings`

**EN:**

This is toilets, sewage, and hygiene—not drinking-water filtration alone (see Clean Water). Sensors without emptying and treatment are theater. Prefer dignity, gender-safe school facilities, and safe sludge economies over luxury smart-toilet hype for the already served.

**FR:**

Il s’agit des toilettes, des eaux usées et de l’hygiène, et non pas uniquement de la filtration de l’eau potable (voir Eau propre). Les capteurs sans vidange ni traitement sont du théâtre. Préférez la dignité, des installations scolaires sans danger pour les hommes et les femmes et des économies de boues sûres au battage médiatique de luxe sur les toilettes intelligentes pour ceux qui sont déjà servis.

---
### `short-termism.currentState`

**EN:**

Households and communities under pressure still liquidate long-term assets for immediate survival: farmers slaughter dairy cows in famine; forests are burned for this season’s field. The same pattern appears in politics and firms that raid the future for short returns.

**FR:**

Les ménages et les communautés sous pression continuent de liquider leurs actifs à long terme pour survivre immédiatement : les agriculteurs abattent leurs vaches laitières en cas de famine ; les forêts sont brûlées pour le champ de cette saison. Le même schéma apparaît dans la politique et les entreprises qui pillent l’avenir à la recherche de rendements à court terme.

---
### `short-termism.rootCauses`

**EN:**

Poverty traps and missing safety nets force high discount rates; insecure land and resource rights; markets and politics that reward this quarter’s yield over decades of soil, forest, and resilience capital.

**FR:**

Les pièges de la pauvreté et l’absence de filets de sécurité imposent des taux d’actualisation élevés ; l'insécurité des droits fonciers et des ressources ; des marchés et des politiques qui récompensent le rendement de ce trimestre au fil de décennies de capital en matière de sols, de forêts et de résilience.

---
### `short-termism.warnings`

**EN:**

Related to poverty, but the core problem is temporal tradeoff. Invent incentives and buffers that make protecting tomorrow rational today—not lectures that ignore empty stomachs. Avoid “green” schemes that lock the poor out of livelihoods without alternatives.

**FR:**

Lié à la pauvreté, mais le problème central est le compromis temporel. Inventez des incitations et des tampons qui rendent la protection de demain rationnelle aujourd’hui – et non des conférences qui ignorent les estomacs vides. Évitez les programmes « verts » qui excluent les pauvres de moyens de subsistance sans alternatives.

---
### `slavery.currentState`

**EN:**

Chattel slavery is illegal nearly everywhere, but forced labor, debt bondage, trafficking, and child labor persist in supply chains, domestic work, fishing, and conflict zones—often hidden from consumers.

**FR:**

L’esclavage mobilier est illégal presque partout, mais le travail forcé, la servitude pour dettes, la traite et le travail des enfants persistent dans les chaînes d’approvisionnement, le travail domestique, la pêche et les zones de conflit, souvent à l’abri des consommateurs.

---
### `slavery.rootCauses`

**EN:**

Profit from cheap coerced labor, weak enforcement, migration vulnerability, corruption, and opaque multi-tier supply chains that dilute accountability.

**FR:**

Profitez d’une main d’œuvre forcée et bon marché, d’une application peu rigoureuse, de la vulnérabilité migratoire, de la corruption et de chaînes d’approvisionnement opaques à plusieurs niveaux qui diluent la responsabilité.

---
### `slavery.warnings`

**EN:**

Worker surveillance marketed as “compliance” can harm the people you claim to protect. Design with worker voice, safe reporting, and remedy—not only blockchain proofs for brands.

**FR:**

La surveillance des travailleurs commercialisée sous le nom de « conformité » peut nuire aux personnes que vous prétendez protéger. Concevez avec la voix des travailleurs, des rapports sécurisés et des solutions, pas seulement des preuves de blockchain pour les marques.

---
### `smoking.currentState`

**EN:**

Over a billion people still use tobacco regularly; smoking kills millions yearly and remains concentrated in lower-income countries even as high-income rates fall. Secondhand smoke and youth nicotine uptake (including new products) keep the epidemic alive.

**FR:**

Plus d’un milliard de personnes consomment encore régulièrement du tabac ; le tabagisme tue des millions de personnes chaque année et reste concentré dans les pays à faible revenu, même si les taux de revenus élevés diminuent. La fumée secondaire et la consommation de nicotine chez les jeunes (y compris les nouveaux produits) entretiennent l’épidémie.

---
### `smoking.rootCauses`

**EN:**

Addiction biology, industry marketing, retail density, weak cessation services, and lagging regulation where enforcement is thin. Social norms and stress keep initiation high among youth in some places.

**FR:**

Biologie de la toxicomanie, marketing industriel, densité de vente au détail, faiblesse des services de sevrage et réglementation en retard là où l'application est mince. Les normes sociales et le stress maintiennent l’initiation parmi les jeunes dans certaines régions.

---
### `smoking.warnings`

**EN:**

Shame-only campaigns fail people with least access to cessation. Invent for initiation prevention, affordable quit support, and secondhand protection—not surveillance of smokers without care. Avoid industry-capture “solutions.”

**FR:**

Les campagnes axées uniquement sur la honte échouent auprès des personnes ayant le moins accès au sevrage. Inventez pour la prévention de l’initiation, une aide abordable pour arrêter de fumer et une protection de seconde main – et non pour la surveillance des fumeurs sans soins. Évitez les « solutions » de capture de l’industrie.

---
### `totalitarianism.currentState`

**EN:**

Strict totalitarian systems still isolate populations through repression, indoctrination, and information monopoly. Broader autocracy has expanded; tools that help people access free media, communicate, and organize are scarce where they are needed most.

**FR:**

Des systèmes totalitaires stricts isolent encore les populations par la répression, l’endoctrinement et le monopole de l’information. Une autocratie plus large s’est développée ; les outils qui aident les gens à accéder aux médias libres, à communiquer et à s’organiser sont rares là où ils sont le plus nécessaires.

---
### `totalitarianism.rootCauses`

**EN:**

Concentrated coercive power, state media monopolies, censorship infrastructure, fear of association, and international free-riding when isolated populations cannot be reached safely.

**FR:**

Pouvoir coercitif concentré, monopoles d’État sur les médias, infrastructures de censure, peur de l’association et parasitisme international lorsque les populations isolées ne peuvent être atteintes en toute sécurité.

---
### `totalitarianism.warnings`

**EN:**

People need free media, language bridges, censorship circumvention, and safer collective action—not better state surveillance. Dual-use is extreme: tools can be seized. Prefer sousveillance and mutual aid designs that do not require a friendly regime.

**FR:**

Les gens ont besoin de médias libres, de ponts linguistiques, de contournement de la censure et d’une action collective plus sûre – et non d’une meilleure surveillance de l’État. Le double usage est extrême : les outils peuvent être saisis. Préférez les conceptions de sousveillance et d’entraide qui ne nécessitent pas de régime amical.

---
### `waste.currentState`

**EN:**

Cities generate on the order of two billion tons of municipal solid waste yearly, plus huge e-waste and hazardous streams. Recycling and circular systems grow but lag production; landfills, open dumps, and plastic-clogged drains still dominate many places.

**FR:**

Les villes génèrent chaque année environ deux milliards de tonnes de déchets solides municipaux, auxquels s’ajoutent d’énormes flux de déchets électroniques et dangereux. Les systèmes de recyclage et circulaires se développent mais sont à la traîne de la production ; Les décharges, les décharges à ciel ouvert et les canalisations bouchées par le plastique dominent encore de nombreux endroits.

---
### `waste.rootCauses`

**EN:**

Linear production, cheap single-use packaging, weak collection, informal-sector exclusion from formal plans, and global waste trade that shifts harm to poorer regions.

**FR:**

Production linéaire, emballages à usage unique bon marché, collecte faible, exclusion du secteur informel des plans formels et commerce mondial des déchets qui déplace les dommages vers les régions les plus pauvres.

---
### `waste.warnings`

**EN:**

Do not invent “export the trash” as local success. Design with informal pickers’ livelihoods, real material recovery, and reduction at source. E-waste burning and plastic-choked floods need systems, not one-off beach cleanups as the whole invent.

**FR:**

N’inventez pas « l’exportation des déchets » comme une réussite locale. Concevoir en tenant compte des moyens de subsistance des cueilleurs informels, d’une véritable récupération des matériaux et d’une réduction à la source. L’incinération des déchets électroniques et les inondations étouffées par le plastique nécessitent des systèmes, et non des nettoyages ponctuels des plages comme l’ensemble l’invente.

---
### `water.currentState`

**EN:**

Billions still lack safely managed drinking water or sanitation. Climate stress, pollution, and over-extraction hit wells and utilities; tanker economies fill gaps at high cost.

**FR:**

Des milliards de personnes manquent toujours d’eau potable ou d’assainissement gérés en toute sécurité. Le stress climatique, la pollution et la surexploitation frappent les puits et les services publics ; les économies de pétroliers comblent les lacunes à un coût élevé.

---
### `water.rootCauses`

**EN:**

Underinvestment in infrastructure, pollution, aquifer depletion, weak governance of shared waters, and poverty that prices households out of safe supply.

**FR:**

Le sous-investissement dans les infrastructures, la pollution, l’épuisement des aquifères, la mauvaise gouvernance des eaux partagées et la pauvreté qui prive les ménages d’un approvisionnement sûr.

---
### `water.warnings`

**EN:**

Pumps without maintenance budgets fail. Design for local repair, energy for treatment, and fair allocation—not one-shot hardware drops.

**FR:**

Les pompes sans budget de maintenance tombent en panne. Concevez pour une réparation locale, de l'énergie pour le traitement et une allocation équitable, et non pour des chutes de matériel ponctuelles.

---
### `weather.currentState`

**EN:**

Heat waves, floods, storms, and droughts are more frequent or intense in many regions under climate change. Early warning has improved, but last-mile alerts, infrastructure, and recovery funds still fail the poorest districts first.

**FR:**

Les vagues de chaleur, les inondations, les tempêtes et les sécheresses sont plus fréquentes ou plus intenses dans de nombreuses régions soumises au changement climatique. L’alerte précoce s’est améliorée, mais les alertes du dernier kilomètre, les infrastructures et les fonds de relance ne parviennent toujours pas à cibler en priorité les districts les plus pauvres.

---
### `weather.rootCauses`

**EN:**

Warmer atmosphere and oceans load the dice for extremes; exposure grows with coastal and urban expansion; underinvestment in resilient infrastructure and social safety nets.

**FR:**

Une atmosphère et des océans plus chauds donnent le ton aux extrêmes ; l'exposition augmente avec l'expansion côtière et urbaine ; le sous-investissement dans des infrastructures résilientes et des filets de sécurité sociale.

---
### `weather.warnings`

**EN:**

Sensors without response plans are theater. Invent for warning-to-action loops, cooling/flood shelter, and equity—not only prettier dashboards. Timing matters: deploy usable pilots before the next season, not perfect models after.

**FR:**

Les capteurs sans plan de réponse sont du théâtre. Inventez des boucles d'alerte à l'action, des abris contre les inondations et le refroidissement et l'équité, et pas seulement des tableaux de bord plus jolis. Le timing compte : déployez des pilotes utilisables avant la saison prochaine, pas des modèles parfaits après.

---
### `women.currentState`

**EN:**

Legal equality has advanced in many countries, yet gaps remain in safety, pay, political power, education access, and reproductive rights. Backlash and uneven enforcement reverse gains in some places.

**FR:**

L’égalité juridique a progressé dans de nombreux pays, mais des écarts subsistent en matière de sécurité, de rémunération, de pouvoir politique, d’accès à l’éducation et de droits reproductifs. Les réactions négatives et une application inégale annulent les gains dans certains endroits.

---
### `women.rootCauses`

**EN:**

Patriarchal norms, economic dependence, violence and legal barriers, underrepresentation in institutions, and underinvestment in care infrastructure that falls disproportionately on women.

**FR:**

Normes patriarcales, dépendance économique, violence et obstacles juridiques, sous-représentation dans les institutions et sous-investissement dans les infrastructures de soins qui pèsent de manière disproportionnée sur les femmes.

---
### `women.warnings`

**EN:**

Tech that ignores safety (e.g., public location sharing without controls) can increase harm. Center agency, privacy, and local women’s leadership—not savior gadgets designed elsewhere.

**FR:**

Les technologies qui ignorent la sécurité (par exemple, le partage d’un emplacement public sans contrôle) peuvent accroître les dommages. L’agence, la vie privée et le leadership des femmes locales – pas des gadgets salvateurs conçus ailleurs.

---
### `women-stem.currentState`

**EN:**

Only about 35% of STEM students in higher education globally are women. Girls often perform well early, then leave math and science pathways at secondary and tertiary transitions; workplace retention in labs and tech roles remains unequal.

**FR:**

Seulement environ 35 % des étudiants STEM dans l’enseignement supérieur dans le monde sont des femmes. Les filles obtiennent souvent de bons résultats très tôt, puis abandonnent les filières mathématiques et scientifiques lors des transitions secondaire et tertiaire ; la rétention sur le lieu de travail dans les laboratoires et les rôles technologiques reste inégale.

---
### `women-stem.rootCauses`

**EN:**

Stereotypes, unsafe or distant schools, household labor, hostile lab cultures, missing mentors and role models, and care infrastructure that falls on women after degrees. This is a STEM pipeline problem—not the whole of equal rights.

**FR:**

Stéréotypes, écoles dangereuses ou éloignées, travail domestique, cultures de laboratoire hostiles, mentors et modèles manquants, et infrastructures de soins qui incombent aux femmes après leurs diplômes. Il s’agit d’un problème de pipeline STEM, et non de l’ensemble de l’égalité des droits.

---
### `women-stem.warnings`

**EN:**

Do not invent “one inspirational app” as the whole solution. Prefer safe transit, lab culture, scholarships with placement, and care near workplaces. Distinct from general women’s rights inventing (safety, law, political voice)—keep the focus on science and engineering pathways.

**FR:**

N’inventez pas « une application inspirante » comme solution globale. Préférez les transports en commun sécuritaires, la culture du laboratoire, les bourses avec placement et les soins à proximité des lieux de travail. À la différence de l’invention générale des droits des femmes (sécurité, droit, voix politique) : restez concentré sur les filières scientifiques et techniques.

---
