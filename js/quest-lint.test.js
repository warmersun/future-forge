...
function lint(tile) {
  ...
  const validated = v.ok ? v : { ok: false, details: v.details, mission: tile.mission || {}, tile };
  return lintQuestTile(tile, validated).warnings.map((w) => w.code);
}
