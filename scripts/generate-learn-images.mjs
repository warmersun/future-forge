/**
 * Pre-generate Learn-dialog stills into assets/learn/{techId}/{sectionId}.jpg
 * Uses SuperGrok session (~/.grok/auth.json) or FF_XAI_API_KEY — same as
 * generate-market-news-images.mjs.
 *
 * Usage:
 *   node scripts/generate-learn-images.mjs
 *   node scripts/generate-learn-images.mjs --only=ai,drones
 *   node scripts/generate-learn-images.mjs --force
 *   node scripts/generate-learn-images.mjs --limit=5
 *   node scripts/generate-learn-images.mjs --concurrency=2
 */
...
import { learnImageJobs } from "../js/learn-stack.js";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const ROOT = path.join(__dirname, "..");
const OUT_DIR = path.join(ROOT, "assets/learn");
...
async function main() {
  ...
  let items = learnImageJobs(TECHS);
  if (opts.only) {
    items = items.filter(
      (x) => opts.only.has(x.techId) || opts.only.has(x.id) || opts.only.has(x.sectionId)
    );
  }
  ...
  console.log(
    `Learn stills: ${items.length} selected, ${todo.length} to generate (force=${opts.force}, concurrency=${opts.concurrency})`
  );
  if (!todo.length) {
  ...
  await mapPool(todo, opts.concurrency, async (item) => {
    ...
    try {
      const buf = await xaiImageGenerate(item.prompt);
